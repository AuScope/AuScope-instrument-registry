...
def validate_instrument_depth(instrument_df):
    errors = []

    filtered_df = instrument_df.dropna(subset=["depth_from", "depth_to"])
    # Check if depth_from and depth_to are valid numbers
    for column in ["depth_from", "depth_to"]:
        non_empty = instrument_df[column].notna() & (instrument_df[column].astype(str).str.strip() != '')
        non_numeric = instrument_df[non_empty & pd.to_numeric(instrument_df[column].astype(str).str.strip(), errors='coerce').isna()]
...
def validate_elevation(instrument_df):
    errors = []
    # Filter out empty cells
    non_empty_elevations = instrument_df["elevation"].dropna().replace('', pd.NA).dropna()
    ...
    for index, value in invalid_rows.items():
        original_value = instrument_df.loc[index, "elevation"].strip()
        errors.append(f"Row {index}: Elevation '{original_value}' is not a valid number")
    return errors


def validate_match_related_resource_url(instrument_df, resource_df):
    errors = []

    # Extract unique URLs from instrument_df
    instrument_urls = set()
    for cell in instrument_df['related_resources_urls']:
        urls = set(url.strip() for url in cell.split(';') if url.strip())
        instrument_urls.update(urls)
    ...
    if instrument_urls != resource_urls:
        missing_in_instrument = resource_urls - instrument_urls
        missing_in_resource = instrument_urls - resource_urls

        if missing_in_instrument:
            errors.append("The following related resource URLs are not in the instrument sheet:\n" + ', '.join(missing_in_instrument))

        if missing_in_resource:
            errors.append("The following instrument URLs are not in the related resource sheet:\n" + ', '.join(missing_in_resource))

    return errors

def validate_match_user_email(instrument_df, author_df):
    errors = []
    instrument_emails = set()
    for cell in instrument_df['author_emails']:
        if isinstance(cell, str):
            emails = set(email.strip().lower() for email in cell.split(';') if email.strip())
            instrument_emails.update(emails)
    ...
    if instrument_emails != author_emails:
        missing_in_instrument = author_emails - instrument_emails
        missing_in_author = instrument_emails - author_emails

        if missing_in_instrument:
            errors.append("The following author emails are not in the instrument sheet:\n" + ', '.join(missing_in_instrument))

        if missing_in_author:
            errors.append("The following instrument emails are not in the author sheet:\n" + ', '.join(missing_in_author))

    return errors

def validate_match_project_identifier(instrument_df, project_df):
    errors = []

    # Extract unique project IDs from instrument_df
    instrument_project_ids = set()
    for cell in instrument_df['project_ids']:
        if pd.notna(cell) and cell.strip() != '':
            ids = set(id.strip() for id in cell.split(';') if id.strip())
            instrument_project_ids.update(ids)
    ...
    if instrument_project_ids != project_identifiers:
        missing_in_instrument = project_identifiers - instrument_project_ids
        missing_in_project = instrument_project_ids - project_identifiers

        if missing_in_instrument:
            errors.append("The following project identifiers are not in the instrument sheet:\n" + ', '.join(missing_in_instrument))

        if missing_in_project:
            errors.append("The following project IDs from the instrument sheet are not in the project sheet:\n" + ', '.join(missing_in_project))

    # Optionally, you can add a check to ensure there's at least one project ID/identifier in each dataframe
    if not instrument_project_ids:
        errors.append("The instrument sheet does not contain any valid project IDs.")
    if not project_identifiers:
        errors.append("The project sheet does not contain any valid project identifiers.")

    return errors

def validate_acquisition_date(instrument_df):
    errors = []

    for column in ['acquisition_start_date', 'acquisition_end_date']:
        # Check for missing dates
        missing_dates = instrument_df[column].isna() | (instrument_df[column] == '')
        ...
        non_empty = instrument_df[column].notna() & (instrument_df[column] != '')
        non_date = instrument_df[non_empty & pd.to_datetime(instrument_df[column], errors='coerce', format='%Y-%m-%d').isna()]
    ...
    for column in ['acquisition_start_date', 'acquisition_end_date']:
        instrument_df[column] = pd.to_datetime(instrument_df[column], errors='coerce', format='%Y-%m-%d').dt.strftime('%Y-%m-%d')

    # Filter for rows where both dates are not null
    valid_df = instrument_df.dropna(subset=['acquisition_start_date', 'acquisition_end_date'])
...
def validate_parent_instruments(df):
    errors = []
    ...
    # Create a dictionary of instruments in the current DataFrame
    current_instruments = dict(zip(df['instrument_number'], df['acquisition_start_date']))

    # Get list of all packages in CKAN
    try:
        package_list = toolkit.get_action('package_list')({}, {})
        ckan_instruments = {}
        organisation_instrument_numbers = []
        for package in package_list:
            package_data = toolkit.get_action('package_show')({}, {'id': package})
            instrument_number = package_data.get('instrument_number')
            if instrument_number:
                ckan_instruments[instrument_number] = pd.to_datetime(package_data.get('acquisition_start_date'))
                organisation_instrument_numbers.append(instrument_number)
    except Exception as e:
        errors.append(f"Error fetching CKAN data: {str(e)}")
        raise ValueError("\n".join(errors))

    def validate_instrument(row):
        if pd.isnull(row['parent_instrument']) or row['parent_instrument'] == '':
            return True

        if row['parent_instrument'] == row['instrument_number']:
            errors.append(f"Instrument {row['instrument_number']} references itself as parent")
            return False

        parent_start_date = None
        if row['parent_instrument'] in current_instruments:
            parent_start_date = current_instruments[row['parent_instrument']]
        elif row['parent_instrument'] in ckan_instruments:
            parent_start_date = ckan_instruments[row['parent_instrument']]
        else:
            errors.append(f"Parent instrument {row['parent_instrument']} for instrument {row['instrument_number']} not found in the instrument repository")
            return False
        ...
        if row['acquisition_start_date'] < parent_start_date:
            errors.append(f"Instrument {row['instrument_number']} : {row['acquisition_start_date']} start date is earlier than its parent {row['parent_instrument']} : {parent_start_date}")
            return False

        return True

    df['valid_parent'] = df.apply(validate_instrument, axis=1)

    # Additional validation from validate_parent function
    instrument_numbers = df["instrument_number"].tolist()
    for index, row in df.iterrows():
        parent = row["parent_instrument"]
        if pd.isna(parent) or parent == '':
            continue
        if parent not in instrument_numbers and parent not in organisation_instrument_numbers:
            errors.append(f"Parent instrument '{parent}' not found in the uploaded instruments or the organisation.")
...
def check_unique_instrument_number(instruments_df):
    errors = []
    duplicates = instruments_df[instruments_df['instrument_number'].duplicated(keep=False)]
    if not duplicates.empty:
        duplicate_values = duplicates['instrument_number'].value_counts()
        errors.append(f"Duplicate instrument numbers detected: {duplicate_values.index.tolist()}")
    return errors

def validate_epsg(instruments_df):
    errors = []
    ...
    invalid_latitudes = instruments_df['point_latitude'].apply(lambda x: not is_cell_empty(x.strip() if isinstance(x, str) else x) and not is_numeric(x.strip() if isinstance(x, str) else x))
    invalid_longitudes = instruments_df['point_longitude'].apply(lambda x: not is_cell_empty(x.strip() if isinstance(x, str) else x) and not is_numeric(x.strip() if isinstance(x, str) else x))


    # Raise an error if any invalid rows are found
    if invalid_latitudes.any():
        errors.append(f"Invalid point_latitude values:\n{instruments_df[invalid_latitudes]}")
    if invalid_longitudes.any():
        errors.append(f"Invalid point_longitude values:\n{instruments_df[invalid_longitudes]}")


    # Check for missing EPSG when coordinates are provided
    missing_epsg = instruments_df[
        (instruments_df['point_latitude'].apply(lambda x: not is_cell_empty(x))) &
        (instruments_df['point_longitude'].apply(lambda x: not is_cell_empty(x))) &
        (instruments_df['epsg_code'].apply(is_cell_empty))
    ]
    if not missing_epsg.empty:
        errors.append("EPSG is required when both point_latitude and point_longitude are specified.")

    invalid_epsg = instruments_df[
        instruments_df['epsg_code'].apply(lambda x: not is_cell_empty(x) and x not in valid_epsg_codes)
    ]
...
def validate_related_resources(related_resources_df):
    ...
    valid_resource_types = ["Audiovisual", "Book", "BookChapter", "Organisation", "ComputationalNotebook", "ConferencePaper", "ConferenceProceeding", "DataPaper", "Dataset", "Dissertation", "Event", "Image", "Instrument", "InteractiveResource", "Journal", "JournalArticle", "Model", "Other", "OutputManagementPlan", "PeerReview", "PhysicalObject", "Preprint", "Report", "Service", "Software", "Sound", "Standard", "StudyRegistration", "Text", "Workflow"]
...
def validate_author_identifier(authors_df, valid_identifier_types):
    ...
    return errors
def validate_instrument_type(instrument_df):
    errors = []
    instrument_types = [
        ...
        "Grab", "Heavy Mineral Concentrate", "Individual Instrument", "Litter", "Other",
    ...
    valid_instruments = instrument_df[instrument_df['instrument_type'].notna() & (instrument_df['instrument_type'] != '')]

    invalid_types = valid_instruments[~valid_instruments['instrument_type'].isin(instrument_types)]
    if not invalid_types.empty:
        invalid_type_list = invalid_types['instrument_type'].unique().tolist()
        errors.append(f"Invalid instrument types found: {invalid_type_list}. Must be one of: {', '.join(instrument_types)}")
...
def validate_authors(authors_df):
    ...
    return errors
def generate_instrument_name(org_id, instrument_type, instrument_number):
    ...
    return name
def generate_instrument_title(org_id, instrument_type, instrument_number):
    org_name= get_organization_name(org_id)
    org_name = org_name
...
def get_organization_name(organization_id):
    try:
        ...
        return None
def validate_instrument_names(instruments_df, org_id):
    instruments_data = []
    existing_names = set()
    errors = []
    for _, row in instruments_df.iterrows():
        ...
        if instrument["name"] in existing_names:
            errors.append(f"Duplicate instrument name: {instrument['name']}")
        else:
            existing_names.add(instrument["name"])
        ...
        try:
            ...
            for package in package_list:
                ...
                if existing_name in existing_names:
                    errors.append(f"Instrument name {existing_name} already exists in CKAN")
        except Exception as e:
            errors.append(f"Error fetching CKAN data: {str(e)}")
    return errors

def validate_instruments(instruments_df, related_resources_df, authors_df, funding_df):
    errors = []
    instruments_columns_to_check = ['instrument_number', 'description', 'user_keywords', 'instrument_type', 'author_emails']
    errors.extend(check_required_fields(instruments_df, instruments_columns_to_check))
    errors.extend(check_unique_instrument_number(instruments_df))
    errors.extend(validate_epsg(instruments_df))
    errors.extend(validate_instrument_depth(instruments_df))
    errors.extend(validate_elevation(instruments_df))
    errors.extend(validate_instrument_type(instruments_df))
    errors.extend(validate_match_related_resource_url(instruments_df, related_resources_df))
    errors.extend(validate_match_user_email(instruments_df, authors_df))
    errors.extend(validate_match_project_identifier(instruments_df, funding_df))
    errors.extend(validate_acquisition_date(instruments_df))
